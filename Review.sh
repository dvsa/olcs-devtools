...
do
	if [ -f $file ]
		...
		if [[ ${file: -4} == ".php" ]]
			then
			 $PHPCS --severity=$PHPCS_SEVERITY --standard="${DEVTOOLS}/vendor/olcs/coding-standards/Profiles/DVSA/CS/ruleset.xml" $file
		fi
	fi
done
...
do
	if [ -f $file ]
		...
		if [[ ${file: -4} == ".php" ]]
			then
			 $PHPMD $file "Devtools\CustomTextRenderer" "${DEVTOOLS}/vendor/olcs/coding-standards/Profiles/DVSA/PMD/ruleset.xml"
		fi
	fi
done
